...
import {
  ClipboardList, Briefcase, Hourglass, UserCheck, UserX,
  Trophy, Search, Filter, Download, Eye, UserPlus,
} from "lucide-react";
...
export default function AllApplication() {
  ...
  const [vacancy, setVacancy] = useState("All Vacancies");
  ...
  const navigate = useNavigate();
    ...
    const fetchData = async () => {
        ...
        // Check if the response has a 'data' property (as per your backend structure)
        // If your backend returns the array directly, use response directly
  ...
  const stats = [
    ...
    { title: "Pending", value: applications.filter(a => a.application_status === 'Pending').length, icon: Hourglass, color: "border-orange-400", bg: "bg-orange-50", iconColor: "text-orange-500" },
  ...
  const hanldleViewApplication = (applicationId) => {
            ...
            {loading ? (
              ...
              applications.map((app, idx) => (
                <tr key={idx} className="border-b hover:bg-slate-50">
                  ...
                  <td className="px-5 py-4 font-semibold text-slate-700">{`${app.last_name}, ${app.first_name}`}</td>
                  <td className="px-5 py-4">{app.position_title || "N/A"}</td>
                  <td className="px-5 py-4">{app.date_received}</td>
                  <td className="px-5 py-4 text-center">
                    <span className="rounded-full px-4 py-1 text-sm bg-blue-100 text-blue-700">{app.application_status}</span>
                  </td>
                  <td className="px-5 py-4 text-center">
                    <div className="flex items-center gap-2">
    
                    <button
                    onClick={() => hanldleViewApplication(app.application_id)}
